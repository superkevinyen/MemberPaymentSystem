...
flowchart TD
    ...
    B --> C[用戶選擇卡片]
    C --> D{卡片狀態檢查}
    D -->|inactive| E[顯示卡片未激活]
    D -->|active| F[調用 rotate_card_qr RPC]
...
flowchart TD
    ...
    B --> C[用戶選擇卡片]
    C --> D{卡片類型檢查}
    D -->|standard/voucher| E[顯示不支持充值]
    D -->|prepaid/corporate| F[輸入充值金額]
    ...
    L --> M{用戶確認}
    M -->|否| N[取消充值]
    M -->|是| O[調用 user_recharge_card RPC]
...
flowchart TD
    ...
    G --> H{用戶確認}
    H -->|否| I[取消收款]
    H -->|是| J[調用 merchant_charge_by_qr RPC]
...
flowchart TD
    ...
    L --> M{用戶確認}
    M -->|否| N[取消退款]
    M -->|是| O[調用 merchant_refund_tx RPC]
